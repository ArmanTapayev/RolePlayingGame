...
public class BattleScene {
    ...
    public void fight(FantasyCharacter hero, FantasyCharacter monster, MainApp.FightCallback fightCallback) {
        //Ходы будут идти в отдельном потоке
        Runnable runnable = () -> {
            int turn = 1;                   // Переменная для записи номера хода
            boolean isFightEnded = false;
            while (!isFightEnded) {
                System.out.println("---- Ход: " + turn + " ----");
                // Персонажи бьют по очереди, поэтому здесь мы описываем логику смены сторон
                if (turn++ % 2 != 0) {
    ...
    private Boolean makeHit(FantasyCharacter defender, FantasyCharacter attacker, MainApp.FightCallback fightCallback) {
        int hit = attacker.attack();                             // Получаем силу удара
        int defenderHealth = defender.getHealthPoints() - hit;   // Отнимаем количество урона из здоровья защищающегося
        ...
        if (defenderHealth <= 0 && defender instanceof Hero) {
            ...
            System.out.println("О горе! Герой пал в бою!");
